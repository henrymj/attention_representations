...
fns=dir('*_original.jpg');

for i=1:length(fns) %This is the number of objects in the directory
    img = imread(fns(i).name);

    scrambled_img = imscrambler(img);

    imwrite(scrambled_img,sprintf('%02d_RGB_scrambled.jpg',i),imgtype);

end

function imScrambled = imscrambler(im,p,rescale)

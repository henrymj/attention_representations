...
SCANNING = False  # False for practice, true for the big show


scan_monitor_details = {"name": "Monitor01", "distance": 81, "resolution": [1024, 768], "width": 56.9}
beh_monitor_details = {"name": "Monitor01", "distance": 70, "resolution": [1024, 768], "width": 40}

# TODO: add a practice flag if necessary?
...
exp_info = {
    ...
    "img_size": 4,  # DVA  # TODO - decide on size once you see them in the scanner?
    "img_loc": [3, 3],  # how far (in DVA) the stim are from fixation along the x and y axes, respectively
    "bg_color": [0, 0, 0],
}

print("EXPECTED TIME...")
print(f'WARMUP: {exp_info["nTRs_skip"] * exp_info["TR_len"]}s')
print(
    f'1 TRIAL (of 9): {exp_info["cueTime"] + exp_info["postCueTime"]}s (cue + wait) + {exp_info["nPres"] * (exp_info["stimTime"])}s ({exp_info["nPres"]} stim pres) + {exp_info["ISIs"].sum()}s (sum of ISIs) + {exp_info["delayTime"]}s (delay time) + {exp_info["ResponseWindow"]}s (response window) = {exp_info["cueTime"] + exp_info["postCueTime"] + (6 * exp_info["stimTime"]) + exp_info["ISIs"].sum() + exp_info["delayTime"] + exp_info["ResponseWindow"]}s'
)
print(f'ITIs: {exp_info["ITIs"].sum()}s')
print(f'COOLDOWN: {exp_info["finalCoolDown"]}s')
total_time = (
    (exp_info["nTRs_skip"] * exp_info["TR_len"])
    + 9
    * (exp_info["cueTime"] + exp_info["postCueTime"] + exp_info["nPres"] * (exp_info["stimTime"]) + exp_info["ISIs"].sum() + exp_info["delayTime"] + exp_info["ResponseWindow"])
    + exp_info["ITIs"].sum()
    + exp_info["finalCoolDown"]
)
print(f'TOTAL: {total_time}s = {total_time/60}m = {total_time/exp_info["TR_len"]} TRs')
...
class Task:
    ...
    def __init__(self, monitor_details, exp_info):
        ...
        self.win = visual.Window(
            [1024, 768], monitor=self.monitor, fullscr=False, units="deg", checkTiming=False
        )  # TODO - switch to full screen, turn on checkTiming

        print(f'PIXELS PER DEGREE: {tools.monitorunittools.deg2pix(1, self.monitor)}')  # TESTING - TODO: talk about whether assuming square pixels is bad, and if to compute things onesself
    ...
    def get_run_info_and_file(self):
        ...
        self.filename = f"sub-{str(run_info['participant ID']).zfill(3)}_ses-{str(run_info['session #']).zfill(2)}_run-{str(run_info['run #']).zfill(2)}_Scanning-{SCANNING}_datetime-{run_info['dateStr']}"
    ...
    def init_cue(self):
        ...
        self.category_images = {cat: sorted([s.split("/")[-1] for s in glob(f"images/{cat}/*")]) for cat in self.exp_info["image_categories"]}

        self.category_cues = {}
        for category in self.exp_info["image_categories"]:
            # add text stim on cardinal axes
            top = visual.TextStim(self.win, text=category[0], pos=(0, 1), color="black", units="deg")
            bottom = visual.TextStim(self.win, text=category[0], pos=(0, -1), color="black", units="deg")
            left = visual.TextStim(self.win, text=category[0], pos=(-1, 0), color="black", units="deg")
            right = visual.TextStim(self.win, text=category[0], pos=(1, 0), color="black", units="deg")
    ...
    def init_stimuli(self):
        ...
        for cat in self.exp_info["image_categories"]:
            ...
            for img in self.category_images[cat]:
                self.images[cat][img] = {
                    "a": visual.ImageStim(self.win, image=f"images/{cat}/{img}/a.jpg", size=(sz, sz), units="deg"),
                    "b": visual.ImageStim(self.win, image=f"images/{cat}/{img}/b.jpg", size=(sz, sz), units="deg"),
                }
        ...
        for cat in self.exp_info["image_categories"]:
            ...
            for img in self.category_images[cat]:
                self.images_scrambled[cat][img] = {
                    "a": visual.ImageStim(self.win, image=f"images_scrambled/{cat}/{img}/a.jpg", size=(sz, sz), units="deg"),
                    "b": visual.ImageStim(self.win, image=f"images_scrambled/{cat}/{img}/b.jpg", size=(sz, sz), units="deg"),
                }
    ...
    def update_trial_locs(self, trial):
        ...
        for i, img in enumerate(trial["images"]):
            ...
            for j, img in enumerate(trial[f"placeholder_imgs_pres_{i}"]):
                img.pos = self.quad_locs[trial[f"placeholder_locations_pres_{i}"][j]]

        trial["test_image"].pos = self.quad_locs[trial["test_location"]]

    def draw_stimuli_pres(self, trial, i):
        trial["images"][i].draw()
        for img in trial[f"placeholder_imgs_pres_{i}"]:
            img.draw()
    ...
    def make_trial(self, set_size, target_category, target_locs, ITI):
        ...
        trial = {
            ...
            "change": self.rng.choice([True, False]),
            "ITI": ITI,
        }
        ...
        for i in range(self.exp_info["nPres"]):
            ...
            trial[f"placeholder_img_names_pres_{i}"] = [c[1] for c in curr_images]
            trial[f"placeholder_locations_pres_{i}"] = curr_locations
            trial[f"placeholder_AorB_pres_{i}"] = curr_abs
            trial[f"placeholder_imgs_pres_{i}"] = []
            for j, image in enumerate(curr_images):
                trial[f"placeholder_imgs_pres_{i}"].append(self.images_scrambled[image[0]][image[1]][curr_abs[j]])
    ...
    def run(self):
        ...
        text_stim.draw()
        self.win.flip()
        _ = self.kb.waitKeys(keyList=["t", "T"])

        # Initialize Master Timestamp once the scanner starts, wait some seconds
        self.masterTimer = masterClock()
        self.draw_fix()
        self.win.flip()
        self.masterTimer.wait(self.exp_info["nTRs_skip"] * self.exp_info["TR_len"])  # TRs to skips
        ...
        df.to_csv(f"data/{self.filename}.tsv")
